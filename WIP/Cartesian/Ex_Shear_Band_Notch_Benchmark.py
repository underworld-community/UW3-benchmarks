...
strain_rate_inv2 = uw.discretisation.MeshVariable("SR", mesh1, 1, degree=1)
dev_stress_inv2 = uw.discretisation.MeshVariable("stress", mesh1, 1, degree=1)
node_viscosity = uw.discretisation.MeshVariable("viscosity", mesh1, 1, degree=1)
...
# +
refLength       = 30.    * u.kilometer

refGravity      = 9.81   * u.meter / u.second**2
refDensity      = 2.7e3  * u.kilogram/u.meter**3

refViscosity    = 1e20 *u.pascal*u.second

refPressure     = (refGravity*refDensity*refLength)
...
KL = refLength.to_base_units()
Kt = refTime.to_base_units()
...
stokes.add_dirichlet_bc(nd(2.5*u.millimeter/u.year), "Left", 0)
stokes.add_dirichlet_bc(0, "Left", 1)
stokes.add_dirichlet_bc(-1*nd(2.5*u.millimeter/u.year), "Right", 0)
...
nodal_visc_calc.smoothing = 1e-5
...
def plotFig():
    if uw.mpi.size == 1:
        ...
        with mesh1.access():
            ...
            pvmesh.point_data["edot"] = strain_rate_inv2.data[:,0]
            pvmesh.point_data["eta"] = node_viscosity.data[:,0]
...
# +
mat_viscosity = np.array([nd(1e20 * u.pascal*u.second), nd(1e24 * u.pascal*u.second)])
...
# NL_visc = C / (2*stokes._Einv2+1.0e-10)


# mat_viscosity = np.array([nd(1e21 * u.pascal*u.second), sympy.Min(NL_visc, nd(1e24 * u.pascal*u.second))])

### Set constants for the viscosity and density of the sinker.
viscBottom        = nd(1e21*u.pascal*u.second)


n = 4
viscTop      = nd(4.75e11*u.pascal*u.second**(1/n))
# viscSlab      = nd(1e14*u.pascal*u.second**(1/n))
# NL_slab = viscSlab * (stokes._Einv2**(1/n-1))
viscTop = viscTop * sympy.Pow(stokes._Einv2, (1/n-1))
# NL_slab = viscSlab * (stokes._Einv2**(1/(n-1)))


mat_viscosity = np.array([viscBottom, viscTop])
...
if uw.mpi.rank == 0:
    print("NL solve complete", flush=True)
